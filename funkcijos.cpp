...
cout<<vargsas.size()+saunuolis.size()-1<<" vargsu ir saunuoliu isvedimas i failus uztruko: "<<diff.count()<<" s\n";
...
while(!tikrinimas(paz)||stoi(paz)<=0)
    ...
    {
    cout<<"Iveskite failo pavadinima(kursiokai.txt): "<<endl;
    cin>>filename;
    }
...
for(int i = 1; i<=num; i++)
{
temp.vardas = "Vardas"+std::to_string(dist(mt));
temp.pavarde = "Pavarde"+std::to_string(dist(mt));
std::uniform_int_distribution<int> dist(1, 10);
for(int k = 0; k<paz; k++)
{
       int p = dist(mt);
...
temp.egz = dist(mt);
fr<<std::setw(20)<<temp.vardas<<std::setw(20)<<temp.pavarde;
for(int j=0; j<paz; j++)
{
    fr<<std::setw(6)<<temp.paz[j];
}
fr<<std::setw(5)<<temp.egz<<endl;
temp.paz.clear();
...
auto end = high_resolution_clock::now();
duration <double> diff = end-start;
cout<<num<<" irasu failo sudarymas uztruko: "<<diff.count()<<" s\n";
